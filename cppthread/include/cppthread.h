...
typedef void* (*thread_func_t)(thread_args_t);
...
/* generalized aliases */
#define thread_id_init(id) id = (thread_id_t*)malloc(sizeof(thread_id_t))
#define thread_handle_init(handle) handle = (thread_handle_t*)malloc(sizeof(thread_handle_t))
#define thread_id_safe_destroy(id) if(id) free(id)
...
class CppThread {
        public:
                ...
                virtual ~CppThread();
        private:
                thread_id_t* tid; // on windows this is DWORD
                thread_func_t func;
                thread_attr_t attr;
                thread_args_t args;
                thread_handle_t* handle; // on linux this is the same as the thread_id_t field
                int threadInit();
